package com.gt.ecif.utils.file.upload;
...
import com.gt.ecif.utils.file.FileProperties;
...
public abstract class AbstractFileUpload {
		...
		try {
			if(uploadFiles(fileMap, request, response, modulePath)){
				...
				if(result==0){
					...
					throw new RuntimeException("插入系统文件表异常,上传失败");
				}
				
			}else{
				throw new RuntimeException("文件不存在或未选择上传文件");
			}
	...
	public void delete(String pid) {
		...
		if(result==0){
			throw new RuntimeException("更新文件状态失败");
		}
	}
