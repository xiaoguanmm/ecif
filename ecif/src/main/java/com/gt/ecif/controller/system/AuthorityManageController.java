...
import com.gt.ecif.entity.Authority;
...
public class AuthorityManageController extends SystemManageController {
	 ...
	 * @return
	 */
	@RequestMapping("/getAllAuthorities")
	public  @ResponseBody ModelMap getAllAuthorities(HttpServletRequest request,HttpServletResponse response){
		ModelMap mm =new ModelMap();
		
		List<Map<String,Object>> authorities = this.authorityService.getAllAuthorities();
		if(authorities != null && authorities.size()>0){
			mm.put("successMsg", "获取权限成功");
			mm.put("authority", authorities);
			return mm;
		}
		mm.put("errMsg", "获取权限失败");
		return mm;
	}
