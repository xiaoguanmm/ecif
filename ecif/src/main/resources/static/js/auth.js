 ...
 * 更新上级权限列表
 */
function updateParentAuthList(){
	var parentIdDom = $("select[name=parentId]");
	var dom = parentIdDom.find("option[value!='']").remove();
	$.ajax({
	    url: "systemManage/getAllAuthorities",
	    type: "POST",
	    async: false,
		success : function(msg) {
			if (msg && msg["successMsg"]) {
				authority = msg.authority;
				$.each(authority,function(i,n){
					$("select[name=parentId]").append('<option value="'+n.pid+'">'+n.name+'</option>');
				});
				layui.use('form',function(){
					var form = layui.form;
					form.render("select");
