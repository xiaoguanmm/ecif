$(function() {
	...
	$('.popup-sign-out0').click(function() {
		layer.confirm('退出ECIF工具管理平台',function(){
	...
	if(menusLength>0){
		...
		for(var i=0;i<menusLength;i++){
			...
			if(icon!=""&&typeof(icon)!="undefined"){
				parentDom.find("i").attr("class","fa fa-home "+icon);
			}
			...
			if(url=="#"){
				...
				parentDom.attr("onclick","openTab('"+path+"','"+name+"','"+path+"')").attr("data-index",idx).text(name);
				var iDom = $("<i class=\"fa fa-home "+icon+"\"></i>");
				parentDom.prepend(iDom);
				// parentDom.attr("href",url);
			}
			//fund系统菜单最多两层 故只判断一次childs子元素
