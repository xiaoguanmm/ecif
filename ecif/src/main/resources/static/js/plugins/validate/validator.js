 ...
 */
(function ($) {
    $.fn.bootstrapValidator.i18n.phoneNum = $.extend($.fn.bootstrapValidator.i18n.phoneNum || {}, {
        noticeMsg: '输入的手机号不合法'
    });
    /**
     *  验证内容长度(包含中文)
     * @type {{validate: (function(*, *, *): {valid: boolean, message: (*|string)})}}
     */
    $.fn.bootstrapValidator.validators.phoneNum = {
    	
        validate: function (validator, $field, options) {
            var fieldName = $field.parents("div[class~=form-div]").find(">label:first").text().replace(/:|：/,"");
            var checkFlg = false;
            if($.trim($field.val()).length>0){
            	 if(/^1[3|4|5|6|7|8|9]\d{9}$/.test($field.val())){
                 	checkFlg = true;
                 }
            }else{
            	checkFlg = true;
            ...
            if (_message) {
                ...
                message = fieldName + " " + $.fn.bootstrapValidator.helpers.format($.fn.bootstrapValidator.i18n.phoneNum.noticeMsg);
            }
            return {valid: checkFlg, message: message};
        }
    };
    ...
    var validator = {
         ...
         * @param fieldName
         */
        enableFieldValidators: function(dom,fieldName){
        	dom.data('bootstrapValidator').enableFieldValidators(fieldName, true);
        },
        /**
         * 禁止校验
         * @param dom
         * @param fieldName
         */
        diabledFieldValidators: function(dom,fieldName){
        	dom.data('bootstrapValidator').enableFieldValidators(fieldName, false);
        }
    }
